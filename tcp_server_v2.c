...
// ----------------------------------------------------------------------
// Thread 2: TCP Client Handler (+ HW timestamp extraction)
// ----------------------------------------------------------------------

static int enable_rx_hw_timestamping(int fd) {
    int flags = 0;

    // Ask kernel to attach timestamps to RX packets; prefer hardware when available.
    flags |= SOF_TIMESTAMPING_RX_HARDWARE;
    flags |= SOF_TIMESTAMPING_RAW_HARDWARE;

    // (Optional fallback) also request software timestamps if HW not available.
...
static ssize_t recv_with_hwts(int fd, void *buf, size_t len, struct timespec *hwts_out) {
    char cbuf[512];
    struct iovec iov = {
        .iov_base = buf,
        .iov_len = len,
    ...
    if (!hwts_out) return n;

    for (struct cmsghdr *cmsg = CMSG_FIRSTHDR(&msg);
         cmsg != NULL;
         cmsg = CMSG_NXTHDR(&msg, cmsg)) {
        if (cmsg->cmsg_level == SOL_SOCKET && cmsg->cmsg_type == SCM_TIMESTAMPING) {
            // Per Linux: SCM_TIMESTAMPING carries 3 timespecs:
            //   ts[0] = software timestamp
            //   ts[1] = "transformed" HW timestamp (if available)
            //   ts[2] = raw HW timestamp (if available)
...
// Same as read_full(), but uses recvmsg() on the first chunk to capture a HW timestamp.
static ssize_t read_full_ts(int fd, void *buf, size_t len, struct timespec *hwts_out) {
    size_t off = 0;
    int first = 1;
    ...
    while (off < len) {
        ssize_t n;
        if (first) {
            n = recv_with_hwts(fd, (char *)buf + off, len - off, hwts_out);
            first = 0;
        } else {
...
static void *client_thread(void *arg) {
    ...
    // Enable timestamping on this connection (best effort).
    if (enable_rx_hw_timestamping(fd) < 0) {
        perror("[tcp_thread] setsockopt(SO_TIMESTAMPING)");
        // Continue anyway; you'll just see hwts=0.0
    }

    for (;;) {
        uint64_t seq;
        uint32_t len_n, len;
        struct timespec hwts = {0, 0};

        // 1. Read Seq (8 bytes) + capture timestamp from the packet we recv() first.
        if (read_full_ts(fd, &seq, sizeof(seq), &hwts) <= 0) break;
        ...
        if (seq <= g_frontier) {
            printf("[tcp_thread] Ignored %" PRIu64 " (Duplicate/Late) hwts=%ld.%09ld\n",
                   seq, (long)hwts.tv_sec, hwts.tv_nsec);
        } else if (seq_set_find(g_dropped_set, seq)) {
            printf("[tcp_thread] Recv %" PRIu64 " but was marked DROPPED (Correction) hwts=%ld.%09ld\n",
                   seq, (long)hwts.tv_sec, hwts.tv_nsec);
            ...
            printf("[tcp_thread] Recv %" PRIu64 " hwts=%ld.%09ld\n",
                   seq, (long)hwts.tv_sec, hwts.tv_nsec);
            seq_set_add(&g_received_set, seq);
            advance_frontier_locked();
        }
