...
int xdp_tcp_msg_seq(struct xdp_md *ctx)
{
    ...
    *seq_ptr = seq + 1;
    ...
    // 6. Make some packets invalid at IP layer so kernel drops them
    //    Condition: IP total length < IP header length → drop in ip_rcv().
    if ((seq % 10) == 0) {
        __u16 tot_len = bpf_ntohs(iph->tot_len);
        __u16 ip_hdr_len = (__u16)(ihl * 4);

        // Only corrupt if current tot_len is sane
        if (tot_len >= ip_hdr_len) {
            // Set total length to smaller than header length
            __u16 bad_tot_len = ip_hdr_len - 4; // still >0 and < ip_hdr_len
            iph->tot_len = bpf_htons(bad_tot_len);
            // We DONT fix the IP checksum on purpose; packet should be dropped.
        }
    }
